...
public class ItemPlacementHelper
{
    ...
    public Vector2? GetItemPlasementPosition(PlacementType placementType, int iterationsMax,
        Vector2Int size, bool addOffset)
    {
        ...
        while (iteration < iterationsMax)
        {
            ...
            if (itemArea > 1)
            {
                var (result, plasementPositions) = PlaceBigItem(position, size, addOffset);

                if (result == false)
                    continue;

                tileByType[placementType].ExceptWith(plasementPositions);
                tileByType[PlacementType.NearWall].ExceptWith(plasementPositions);
            }
            else
            {
